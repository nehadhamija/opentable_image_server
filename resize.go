...
func (s *Server) handleNewUpload(w http.ResponseWriter, r *http.Request) {
	...
	if MessageType == "SubscriptionConfirmation" {
		go func() {
			...
			s.snsClient.ConfirmSubscription(&sns.ConfirmSubscriptionInput{
...
func (s *Server) handleUploadUrl(w http.ResponseWriter, r *http.Request) {
	uuid, _ := uuid.NewV4()
	fmt.Fprint(w, fmt.Sprintf("https://s3.%s.amazonaws.com/%s/%s.jpg", s.Config.Region, s.Config.Input_bucket, uuid.String()))
}
